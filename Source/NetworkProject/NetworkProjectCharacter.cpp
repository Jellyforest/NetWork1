...
#pragma region PlayerInfo
	...
	FString gmString = GetWorld()->GetAuthGameMode() != nullptr ? FString("Has GamemodeBase") : FString("Has mo GameModeBase");
...
#pragma endregion
	...
	{
		//infoText = GetPlayerState()->GetPlayerName();
...
void ANetworkProjectCharacter::ChangeSpectatorMode()
{
	...
	if (gm != nullptr)
	{
		//게임모드에 설정한 관전자 폰클래스를 불러온다
		TSubclassOf<ASpectatorPawn> spectatorPawn = gm->SpectatorClass;
		
		FActorSpawnParameters param;
		param.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
		///////ABattleSpectatorPawn* spectator = GetWorld()->SpawnActor <ABattleSpectatorPawn>(spectatorPawn, GetActorLocation(), GetActorLocation(), param);
...
void ANetworkProjectCharacter::DestroyMySession()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("EndSessionResult : %s"), endResult ? TEXT("Success") : TEXT("Failed"));
...
void ANetworkProjectCharacter::EndSession()
{
	...
	if (HasAuthority())
	{
		for (TActorIterator<ANetworkProjectCharacter> pl(GetWorld()); pl; ++pl)
		{
			...
			if (p != this)
			{
				pl->ServerDestroyAllSessions();
			}

		}

		//정상적인 방법은 델리게이트로 해야함 이벤트를 받은 횟수가 플레이어 리스트의 (나를 뺀)num-1을 해야함
...
void ANetworkProjectCharacter::MulticastDestroyAllSessions_Implementation()
{
	if (GetController() != nullptr && GetController()->IsLocalController())
	{
		...
		//c1에서 server 캐릭터는 pc가 없음.내 꺼만 하게해주지 말고 상대방 캐릭터에도 하게해줘야함 때문에 각자의 주인ㅌ캑\릭터한테 시켜줘야하ㅓㅁ 방법은 sever 쪽에서 
		ABattlePlayerController* pc = Cast<ABattlePlayerController>(GetController());
		pc->ClientTravel(FString("/Game/Maps/LoginMap"), ETravelType::TRAVEL_Absolute);
	}
}
